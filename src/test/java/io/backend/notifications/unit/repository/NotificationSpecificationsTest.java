...
/**
 * Unit tests for {@link NotificationSpecifications} static factory methods. Each spec is tested in
 * isolation with mocked JPA Criteria API objects.
 */
...
class NotificationSpecificationsTest {
  ...
  @Mock private Path<Channel> channelPath;

  @Mock private Path<LocalDateTime> createdAtPath;

  @Mock private Path<String> titlePath;

  @Mock private Path<String> contentPath;

  @Mock private Path<User> userPath;
  ...
  void createdAfterShouldCreateGreaterThanOrEqualPredicate() {
    ...
    when(cb.greaterThanOrEqualTo(eq(createdAtPath), any(LocalDateTime.class)))
  ...
  void belongsToUserShouldJoinUserAndFilterByEmail() {
    ...
    Specification<Notification> spec =
        NotificationSpecifications.belongsToUser("alice@example.com");
    Predicate result = spec.toPredicate(root, query, cb);
