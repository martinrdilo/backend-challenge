 ...
 *
 * <p>Usage: Notification n = NotificationBuilder.aNotification().withUser(user).build();
 * Notification custom = NotificationBuilder.aNotification() .withUser(user) .withTitle("Alert")
 * .withChannel(Channel.SMS) .build();
 */
public final class NotificationBuilder {
  ...
  /**
   * Builds a Notification entity (without id — JPA will generate it on persist). Requires a User to
   * be set via withUser().
   */
  public Notification build() {
    if (user == null) {
      throw new IllegalStateException(
          "Notification requires a User. Call withUser() before build().");
    }
