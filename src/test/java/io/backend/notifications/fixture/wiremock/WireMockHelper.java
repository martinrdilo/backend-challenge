 ...
 *
 * <p>All stubs match on PATH only (urlPathEqualTo), making them URL-agnostic. Whether the base URL
 * is localhost or api.jsonplaceholder.com, stubs will match as long as the path after "/" is
 * correct.
 */
public final class WireMockHelper {
  ...
  /**
   * Stub GET /posts to return a list of posts for a given userId. Matches: any base URL + /posts
   * with query param userId.
   */
  ...
  /**
   * Reset all WireMock stubs and request logs. Call this in @BeforeEach to ensure test isolation.
   */
  ...
  /**
   * Stub GET /photos/{id} to return a photo response. Used by notification enrichment to resolve
   * attachment IDs into ExternalPhotoResponse.
   */
  public static void stubGetPhoto(WireMockServer server, long photoId, String responseBody) {
    server.stubFor(
        WireMock.get(urlPathEqualTo("/photos/" + photoId))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withHeader("Content-Type", "application/json")
