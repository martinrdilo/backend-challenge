...
/**
 * Integration tests verifying retry behavior on {@link ChannelDispatcher#dispatch} when annotated
 * with {@code @Retryable}.
 *
 * <p>Tests call {@code channelDispatcher.dispatch(notification)} directly to isolate retry behavior
 * from the async event dispatch layer.
 */
...
class ChannelDispatcherRetryTest extends AbstractIntegrationTest {
  ...
  @Autowired private ChannelDispatcher channelDispatcher;
  ...
  void shouldRetryThreeTimesAndFailWhenExhausted() {
    doThrow(new RuntimeException("Always failing")).when(TestConfig.emailMock).send(any());
  ...
  void shouldNotRetryOnIllegalStateException() {
    doThrow(new IllegalStateException("Validation error")).when(TestConfig.emailMock).send(any());
