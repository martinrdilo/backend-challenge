...
class RateLimitIntegrationTest extends AbstractIntegrationTest {
    ...
    void shouldReturn429ProblemDetailWhenLoginLimitExceeded() {
        ...
        webTestClient().post()
                ...
                .jsonPath("$.title").value(is("Too Many Requests"))
                .jsonPath("$.status").value(equalTo(429))
    ...
    void shouldReturn429ProblemDetailWhenRegisterLimitExceeded() {
        ...
        for (int i = 0; i < REGISTER_LIMIT; i++) {
            RegisterRequest request = UserBuilder.aUser()
                    .withEmail("reg" + i + "@test.com")
        ...
        // The next request should be rate-limited
        RegisterRequest blockedRequest = UserBuilder.aUser()
                .withEmail("blocked@test.com")
                .buildRegisterRequest();
    ...
    @Test
    @DisplayName("should isolate login and register buckets — login exhaustion does not block register")
    void shouldIsolateLoginAndRegisterBuckets() {
        ...
        webTestClient().post()
                .uri("/auth/register")
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(UserBuilder.aUser()
                        .withEmail("fresh@test.com")
                        .buildRegisterRequest())
                .header("X-Forwarded-For", clientIp)
                .exchange()
                .expectStatus().isCreated();
    ...
    void shouldUseXForwardedForHeader() {
        ...
        webTestClient().post()
                ...
                .header("X-Forwarded-For", blockedIp)
                .bodyValue(UserBuilder.aUser()
                        .withEmail("blocked-ip1@test.com")
                        .buildRegisterRequest())
                .exchange()
                .expectStatus().isEqualTo(429);
