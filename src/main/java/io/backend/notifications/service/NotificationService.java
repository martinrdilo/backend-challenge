...
public class NotificationService {
  ...
  /**
   * Searches notifications with optional criteria and pagination. Ownership is enforced via {@link
   * NotificationSpecifications#belongsToUser(String)}. Page size is capped at 100.
   *
   ...
   */
  public Page<EnrichedNotificationResponse> searchNotifications(
      NotificationCriteria criteria, Pageable pageable) {
    String email = SecurityContextHolder.getContext().getAuthentication().getName();
  ...
  public EnrichedNotificationResponse updateNotification(
      Long id, NotificationUpdateRequest request) {
    Notification notification = findOwnNotification(id);
    notification.setTitle(request.title());
    notification.setContent(request.content());
    notification.setAttachmentIds(request.attachmentIds());
  ...
  private List<ExternalPhotoResponse> resolvePhotos(List<Long> ids) {
    ...
    return ids.stream().map(externalMediaClient::getPhotoById).filter(Objects::nonNull).toList();
  }
}
