...
/**
 * Static factory methods for building type-safe {@link Specification} predicates for {@link
 * Notification} queries. Each method returns {@code null} when its parameter is {@code null}, which
 * Spring Data JPA ignores during composition.
 *
 * <p><strong>IDOR protection:</strong> Every query MUST start with {@link #belongsToUser(String)}
 * to enforce ownership isolation.
 */
public final class NotificationSpecifications {
  ...
  /**
   * Filters notifications owned by the user with the given email. Performs a join to the User
   * table.
   *
  ...
  /**
   * Filters notifications created on or after the given date (inclusive). Converts the date to the
   * start of day in {@link LocalDateTime}.
   *
  ...
  /**
   * Filters notifications created on or before the given date (inclusive). Converts the date to the
   * end of day in {@link LocalDateTime}.
   *
  ...
  /**
   * Filters notifications whose title or content contains the given search term (case-insensitive
