...
public class RateLimitFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (LOGIN_PATH.equals(path)) {
      limit =
          Bandwidth.simple(
              config.loginAttemptsPerMinute(), Duration.ofMinutes(config.windowMinutes()));
      bucketStore = loginBuckets;
      resourceName = "login";
    } else {
      limit =
          Bandwidth.simple(
              config.registerAttemptsPerMinute(), Duration.ofMinutes(config.windowMinutes()));
      bucketStore = registerBuckets;
      resourceName = "register";
    }

    Bucket bucket =
        bucketStore.computeIfAbsent(clientIp, ip -> Bucket.builder().addLimit(limit).build());
    ...
    if (bucket.tryConsume(1)) {
      ...
      ProblemDetail problem =
          ProblemDetail.forStatusAndDetail(
              ...
              String.format(
                  "Too many %s attempts. Retry after %ds.", resourceName, retryAfterSeconds));
